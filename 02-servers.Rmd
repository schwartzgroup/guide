...
The following table lists attributes of the servers as of December 18, 2024.

| type    | name    | ip             | CPU  | threads | max clock | memory | capacity |
| :-:     | :-:     | :-:            | :-:  | :-:     | :-:       | :-:    | :-:      |
...
| Work    | gcmc-js | 10.174.192.15  | 11   | 22      | 3.2 GHz   | 756 GB | 116 GB   |
| Storage | Isilon  | 10.174.191.128 | unk. | unk.    | unk.      | unk.   | 245 TB   |
| Storage | qnap2   | 10.174.192.22  | unk. | unk.    | unk.      | unk.   | 29 TB    |
| Storage | qnap3   | 10.174.192.23  | 4    | 4       | 1.7 GHz   | 2 GB   | 44 TB    |
| Storage | qnap5   | 10.174.192.23  | unk. | unk.    | unk.      | unk.   | 26 TB    |
...
- `gate`, `qnap`, and `qnap4` no longer exist and the data has been migrated to the `Isilon` machine. The original mount points of `/media/gate/`, `/media/qnap/`, and `/media/qnap4/` still exist but are just symbolic links to directories in `/media/Isilon/`.
- `Isilon` is shared storage for the whole school. The full storage capacity may not be available.
- `gcmc-js` is a virtual machine, so performance may be degraded compared to the bare metal machines.

## Server resource monitor

[Munin](http://munin-monitoring.org/) is available for resource monitoring purposes. This can be used, for example, to:
...
Munin is hosted on the `dagda` machine due to its low utilization compared to the other machines. To access Munin, navigate to <http://sph-dagda.sph.harvard.edu:8080/munin> <http://10.174.192.12:8080/munin/> in your browser. You will be greeted with the following screen:
